...
import { GridContainer, FormItem, Label, Input, ButtonContainer, InsertModalStyles, DeleteModalStyles, Required, LabelDiv, Select  } from '../../css/componunt/Modalcss.js';
...
function UserManagement() {
  ...
  const handleInsert = async (event) => {
    ...
    if (!userAccountId) {
     ...
     const currentSelectedRoles = Array.from(
      document.querySelectorAll('input.insertrolecheckbox[type="checkbox"]:checked')
  ...
  const handleUpdate = async (event) => {
    ...
    // 현재 체크된 체크박스의 ID를 수집
    const currentSelectedRoles = Array.from(
      document.querySelectorAll('input.updaterolecheckbox[type="checkbox"]:checked')
    ).map((checkbox) => Number(checkbox.value));
  ...
  const [filterList, setFilterList] = useState({user_name:'',user_account_id:''});
  const [UserDetail, setUserDetail] = useState({        //오른쪽 메뉴정보 데이터
    user_id:'-',                         
    user_name:'-',                       
    user_account_id:'-',                 
    user_passwd:'-',                     
  ...
  const fetchUserList = async (page_no, page_cnt, filterList) => {
    try {
      ...
      if (filterList.user_name !== undefined && filterList.user_name !== null && filterList.user_name.trim() !== '') {
        url += `&user_name=${encodeURIComponent(filterList.user_name)}`;
      }
      if (filterList.user_account_id !== undefined && filterList.user_account_id !== null && filterList.user_account_id.trim() !== '') {
        url += `&user_account_id=${encodeURIComponent(filterList.user_account_id)}`;
      }
  ...
  const fetchUserDetail = async () => {
    try {
      const response = await axios.get(`http://localhost:8080/api/admin/user/${user_id}`);
  ...
  const handleUserAccountIdChange = (e) => {
    setFilterList(prevState => ({ ...prevState, user_account_id: e.target.value }));
  };
      ...
      <div className="content_container-ctn2" ref={containerRef}>
        ...
        <div className="read_button-ctn2" id="검색조건">
          <div id="left">
            ...
            사용자ID : <input type="text" placeholder="사용자ID 검색" onChange={handleUserAccountIdChange} />
          </div>
          ...
          <table className ="MainTable">
            ...
            <tbody className="TableBody-ctn2" id ="TableBody" ref={topDivRef}>
              {userList.map((user, index) => (
                <tr key={user.user_id} onClick={() => handleRowClick(index, user.user_id)} className={selectedRow === index ? 'selected' : ''}>
                  <td className="col1">{(currentPage - 1) * itemsPerPage + index + 1}</td> {/* 1부터 시작하는 화면 번호 */}
                  ...
                  <td className="col8">{new Date(user.create_dt).toLocaleString()}</td>
                  <td className="col9">{user.modify_user}</td>
                  <td className="col10">{user.modify_dt ? new Date(user.modify_dt).toLocaleString() : ''}</td>
        ...
        <div className="detailstap-ctn2" ref={bottomDivRef}>
          <Tabs defaultActiveKey="상세정보" id="uncontrolled-tab-example" className="mb-3">
            <Tab eventKey="상세정보" title="상세정보"><UserDetailTable UserDetail={UserDetail} /></Tab>
            <Tab eventKey="역할목록" title="역할목록"><RoleListTable roleList={roleList} relUserRoleList={relUserRoleList} /></Tab>
          </Tabs>
        </div>
      </div>
        ...
        <div>
          ...
          <form onSubmit={handleInsert}>
            <GridContainer>
                ...
                <FormItem key="3">
                  <LabelDiv>
                    ...
                    <Input type="checkbox" id="사용여부" name="user_use_yn" defaultChecked/>
                </FormItem>
                <FormItem key="4">
                  <LabelDiv>
                    <Label htmlFor="사용자구분">사용자구분</Label>
                  </LabelDiv>
                    <Select id="사용자구분" name="user_category" defaultValue="정규">
            ...
            <tbody className="TableBody-ctn2">
              {roleList.map((role) => (
                <tr key={role.role_id}>
                  <td className="col1"><input className='insertrolecheckbox' type='checkbox' value={role.role_id} /></td> 
          ...
          <form onSubmit={handleUpdate}>
            <GridContainer>
                <FormItem key="1">
                  ...
                  <Input type="text" id="사용자명" name="user_name" disabled value={UserDetail.user_name}/>
                </FormItem>
                <FormItem key="2">
                  <LabelDiv>
                    <Label htmlFor="사용자ID">사용자ID</Label>
                  ...
                  <Input type="text" id="사용자ID" name="user_account_id" disabled value={UserDetail.user_account_id}/>
                </FormItem>
                <FormItem key="3">
                  <LabelDiv>
                    <Label htmlFor="사용여부">사용여부</Label>
                  </LabelDiv>
                    <Input type="checkbox" id="사용여부" name="user_use_yn" defaultChecked={UserDetail.user_use_yn === 'Y'}/>
                </FormItem>
                <FormItem key="4">
                  <LabelDiv>
                    <Label htmlFor="사용자구분">사용자구분</Label>
                  </LabelDiv>
                    <Select id="사용자구분" name="user_category" defaultValue={UserDetail.user_category}>
                      <option value="정규">정규</option>
                      <option value="임시">임시</option>
                    </Select>
                </FormItem>
                <FormItem key="5">
                  <LabelDiv>
                    ...
                    <Input type="text" id="부서" name="user_department" value={UserDetail.user_department} onChange={handleInputChange}/>
                </FormItem>
                <FormItem key="6">
                  <LabelDiv>
                    <Label htmlFor="핸드폰">핸드폰</Label>
                  </LabelDiv>
                  <Input type="text" id="핸드폰" name="user_phon_no" value={UserDetail.user_phon_no} onChange={handleInputChange}/>
                </FormItem>
                <FormItem key="7">
                  <LabelDiv>
                    <Label htmlFor="이메일">이메일</Label>
                  </LabelDiv>
                    <Input type="text" id="이메일" name="user_email" value={UserDetail.user_email} onChange={handleInputChange}/>
                </FormItem>
            </GridContainer>
            <table className ="MainTable">
            <thead className="TableHeader">
            ...
            <tbody className="TableBody-ctn2">
              {roleList.map((role) => (
                <tr key={role.role_id}>
                  <td className="col1"><input className='updaterolecheckbox' type='checkbox' value={role.role_id} defaultChecked={isRoleChecked(role.role_id)} /></td> 
                  <td className="col2">{role.role_code}</td>
                  <td className="col3">{role.role_name}</td>
                </tr>
              ))}
